...
$report_type = " - Re-correction";

if($ug_level == '5'){
    $this->load->library('tcpdf/Tcpdf_other');
    $obj_pdf = new Tcpdf_other('L', 'mm', 'A4', true, 'UTF-8', false);
    $obj_pdf->setData($report_type,$year,$semester,$conduct_year,$course_selected,$center_selected);
}
else {
    $this->load->library('tcpdf/Tcpdf_stu');
    $obj_pdf = new Tcpdf_stu('L', 'mm', 'A4', true, 'UTF-8', false);
    $obj_pdf->setData($report_type,$authority_data,$year,$semester,$conduct_year,$course_selected,$center_selected,$effective_date);
//    $obj_pdf = new Tcpdf_stu($authority_data);
    
}

$obj_pdf->SetCreator(PDF_CREATOR);
$title = "Student Re-correction Exam Mark Report";
...
//        $subjects_marks[$va['exam_mark'][$z]['subject_code']] = $va['exam_mark'][$z]['result'];
        $mark = '';
        $rel_result = 0;
        if($va['subjects'][$z]['release_result'] == 1){
            $mark = $va['subjects'][$z]['result'];
        } else{
            $rel_result = 1;
            $mark = "Results not released.";
        }
        $subjects_marks[$va['subjects'][$z]['code']] = $mark;
    }
//    
...
//    $subjects .= '<td align="center">'.$va['grade_point'].'</td>';
