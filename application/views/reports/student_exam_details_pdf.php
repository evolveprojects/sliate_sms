...
$student_approved_subject = array();
...
foreach ($student_details['stu_all_count_array'] as $va) {
...
//    $exam_year = $va['exam_year'];
    ...
    if($stu_status == 1){
        $current_semester = get_Ordinal($va['stu_semester']);
        $current_year = get_Ordinal($va['stu_year']);
    } else {
        $current_semester = get_Ordinal($va['applying_semester']);
        $current_year = get_Ordinal($va['applying_year']);
    }
    
    $student_approved_subject[$count_stu_subject] = $va['subject_id'];
    $count_stu_subject++;

}
$sfs_count = 0;
foreach ($student_details['student_follow_subject'] as $sfs) {
...
foreach ($student_details['common_details'] as $cd) {
    ...
    $common_person .=
        '<td align="left" style="font-family: Times New Roman; font-size:10px;width: 70%;"><label><b>' . $cd['name'] . '</b></label><br><label>' . $cd['position'] . '</label><br><label>SLIATE</label></td>';
...
$content =
    ...
    '</table></font>' .
    '<br>' .
    '<label align="center" style="font-family: Times New Roman; font-size:8px">This candidate is hereby admitted to the above mentioned Examination.</label><br><br><br><br>' .
    ...
    '<label align="center" style="font-family:Times New Roman;font-size:10px">' . $course_name . '</label><br>' .
    '<label align="center" style="font-family:Times New Roman;font-size:10px">' . $current_year . ' Year</label><label style="font-family:Times New Roman;font-size:10px"> (New/Old)</label><br><br>' .
    '<font size="8"><table border="0.5" style="width:100%; vertical-align:center;" cellspacing="0" cellpadding="5">' .
    '<thead>' .
    '<tr>' .
...
// print_r($count_stu_subject_array);
...
function get_subject_version($subject)
{
